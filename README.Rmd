---
output: github_document
---

<!-- README.md is generated from README.Rmd. Please edit that file -->

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.path = "man/figures/README-",
  out.width = "80%"
)
```

# bayeslinear

<!-- badges: start -->
<!-- badges: end -->

<tt>bayeslinear</tt> provides tools to perform a Bayes Linear analysis as well as generalising the solution space to convex subsets. For overview of these methodologies see HERE.

## Installation

<tt>bayeslinear</tt> is currently in development; install the latest version from [GitHub](https://github.com/) with:

``` r
install.packages("devtools")
devtools::install_github("astfalckl/bayeslinear")
```

Also, call in some other packages we need for the README
```{r, warning = FALSE, message = FALSE}
library(bayeslinear)
library(ggplot2)
library(CVXR)
library(dplyr)
library(patchwork)
library(latex2exp)

theme_set(theme_bw())
```

# Standard and Generalised Bayes Linear Analysis

We demonstrate the code by repeating the example given in PAPER.

## Creating Belief Structures

A belief structure is the fundamental unit of information that specifies the geometry of a Bayes linear analysis. We are required to specify $\mathbb{E}(X)$, $\mathbb{E}(D)$, $\mathrm{var}(X)$, $\mathrm{var}(D)$ and $\mathrm{cov}(X,D)$. The methods in <tt>bayeslinear</tt> are based on the creation of a belief structure object through the function <tt>belief_structure()</tt>.

```{r}
nx <- 2
nd <- 2

exp_x <- matrix(c(1, 1))
exp_d <- matrix(c(1, 1))
var_x <- matrix(c(0.54, 0.09, 0.09, 0.54), nrow = nx)
cov_xd <- matrix(c(0.4, -0.1, -.1, -.3), nrow = nx)
var_d <- matrix(c(1, -0.2, -0.2, 1), nrow = nd)

bivariate_bs <- belief_structure(exp_x, exp_d, cov_xd, var_x, var_d)
bivariate_bs
```

## Adjusting Belief Structures

A <tt>belief_structure</tt> object is adjusted by some data <tt>d</tt> via the <tt>adjust</tt> method. An adjusted belief structure <tt>adj_belief_structure</tt> is returned. Note that the initial belief structure is always stored in <tt>prior_bs</tt>.

```{r}
d <- c(3, 6.5)
bivariate_adj_bs <- adjust(bivariate_bs, d)
bivariate_adj_bs
```

## Creating Generalised Belief Structures

We may augment a belief structure with a solution constraint by using the <tt>gen_belief_structure</tt> method. This is similar to above however with the inclusion of a quoted constraint that gets passed to <tt>CVXR</tt>. We demonstrate what some of these constraints look like here, and point towards [here](https://cvxr.rbind.io) for a comprehensive list of examples.

```{r}
bivariate_gbs <- gen_belief_structure(
  exp_x, exp_d, cov_xd, var_x, var_d,
  quote(list(gen_adj_exp >= 0))
)
bivariate_gbs
```

## Adjusting Generalised Belief Structures

This happens with the same <tt>adjust</tt> method as before, but returns the generalised adjusted expectation and variance according to the specified constraint.

```{r}
bivariate_adj_gbs <- adjust(bivariate_gbs, d)
bivariate_adj_gbs
```

## Plotting the results

We recreate Figure 1 from PAPER using the results above. We note that <tt>ggplot</tt> has a habit of being very verbose, and so we have hidden the code from the displayed <tt>README</tt> on GitHub. Please consult the source file for all plotting code used.

```{r, echo = FALSE, warning = FALSE}
adj_exp_text <- tibble(
  x = bivariate_adj_bs$adj_exp[1, ],
  y = bivariate_adj_bs$adj_exp[2, ],
  text = TeX("${E_d}[X]$", output = "character")
)
cadj_exp_text <- tibble(
  x = bivariate_adj_gbs$gen_adj_exp[1, ],
  y = bivariate_adj_gbs$gen_adj_exp[2, ],
  text = TeX("${E_d^C}[X]$", output = "character")
)

x_min <- -1.5
x_max <- 3.5
y_min <- -3.5
y_max <- 1.5

x_axis <- matrix(c(x_max, x_min, 0, 0), nrow = 2) %>% as_tibble()
y_axis <- matrix(c(0, 0, y_max, y_min), nrow = 2) %>% as_tibble()

adj_exp_plot <- ggplot() +
  geom_rect(
    aes(xmin = 0, xmax = x_max, ymin = 0, ymax = y_max),
    fill = "#d8d8d8"
  ) +
  geom_vline(aes(xintercept = 0), alpha = 0.5) +
  geom_hline(aes(yintercept = 0), alpha = 0.5) +
  geom_point(
    data = adj_exp_text,
    aes(x = x, y = y)
  ) +
  geom_text(
    data = adj_exp_text,
    aes(x = x - .3, y = y - .4, label = text),
    parse = TRUE
  ) +
  geom_path(
    data = bayeslinear::calc_ellipse(
      1, bivariate_adj_bs$adj_exp, bivariate_adj_bs$adj_var
    ),
    aes(x = x, y = y), alpha = 0.8
  ) +
  geom_path(
    data = bayeslinear::calc_ellipse(
      2, bivariate_adj_bs$adj_exp, bivariate_adj_bs$adj_var
    ),
    aes(x = x, y = y), alpha = 0.2
  ) +
  geom_line(data = x_axis, aes(x = V1, y = V2), size = 1) +
  geom_line(data = y_axis, aes(x = V1, y = V2), size = 1) +
  scale_x_continuous(
    TeX('$X_1$'),
    limits = c(-1.5, 3.5),
    expand = c(0, 0)
  ) +
  scale_y_continuous(
    TeX('$X_2$'),
    limits = c(-3.5, 1.5),
    expand = c(0, 0)
  ) +
  coord_fixed() +
  theme(panel.grid.minor = element_blank())

  cadj_exp_plot <- ggplot() +
  geom_rect(
    aes(xmin = 0, xmax = x_max, ymin = 0, ymax = y_max),
    fill = "#d8d8d8"
  ) +
  geom_vline(aes(xintercept = 0), alpha = 0.5) +
  geom_hline(aes(yintercept = 0), alpha = 0.5) +
  geom_path(
    data = bayeslinear::calc_ellipse(
      1, bivariate_adj_bs$adj_exp, bivariate_adj_bs$adj_var
    ),
    aes(x = x, y = y), alpha = 0.8 * 0.5
  ) +
  geom_path(
    data = bayeslinear::calc_ellipse(
      2, bivariate_adj_bs$adj_exp, bivariate_adj_bs$adj_var
    ),
    aes(x = x, y = y), alpha = 0.5 * 0.5
  ) +
  geom_point(
    data = cadj_exp_text,
    aes(x = x, y = y)
  ) +
  geom_text(
    data = cadj_exp_text,
    aes(x = x, y = y + 0.7, label = text),
    parse = TRUE
  ) +
  geom_path(
    data = bayeslinear::calc_ellipse(
      1, bivariate_adj_gbs$gen_adj_exp, bivariate_adj_gbs$gen_adj_var
    ),
    aes(x = x, y = y), alpha = 0.8
  ) +
  geom_path(
    data = bayeslinear::calc_ellipse(
      2, bivariate_adj_gbs$gen_adj_exp, bivariate_adj_gbs$gen_adj_var
    ),
    aes(x = x, y = y), alpha = 0.5
  ) +
  geom_line(data = x_axis, aes(x = V1, y = V2), size = 1) +
  geom_line(data = y_axis, aes(x = V1, y = V2), size = 1) +
  scale_x_continuous(
    TeX("$X_1$"),
    limits = c(-1.5, 3.5),
    expand = c(0, 0)
  ) +
  scale_y_continuous(
    TeX("$X_2$"),
    limits = c(-3.5, 1.5),
    expand = c(0, 0)
  ) +
  coord_fixed() +
  theme(panel.grid.minor = element_blank())

u <- svd(bivariate_adj_bs$adj_var)$u
d <- sqrt(diag(svd(bivariate_adj_bs$adj_var)$d))
Linv <- solve(u %*% d)

x_axis <- t(Linv %*% matrix(c(x_max, 0, x_min, 0), nrow = 2)) %>%
    as_tibble()

y_axis <- t(Linv %*% matrix(c(0, 1.79, 0, y_min), nrow = 2)) %>%
    as_tibble()

polygon <- t(
  Linv %*% matrix(c(0, 0, 0, 1.79, x_max, y_max, x_max, 0), nrow = 2)
  ) %>%
    as_tibble()

x_min <- min(-x_axis$V1, -y_axis$V1)
x_max <- max(-x_axis$V1, -y_axis$V1)
y_min <- min(x_axis$V2, y_axis$V2)
y_max <- max(x_axis$V2, y_axis$V2)

polygon[3, 1] <- -x_max
polygon[3, 2] <- y_max

adj_z <- t(Linv %*% bivariate_adj_bs$adj_exp)
gen_adj_z <- t(Linv %*% bivariate_adj_gbs$gen_adj_exp)

adj_z_exp_text <- tibble(
    x = adj_z[, 1],
    y = adj_z[, 2],
    text = TeX("${E_d}[Z]$", output = "character")
)

gen_adj_z_exp_text <- tibble(
    x = gen_adj_z[, 1],
    y = gen_adj_z[, 2],
    text = TeX("${E_d^C}[Z]$", output = "character")
)

gen_adj_var_z <- t(solve(chol(bivariate_adj_bs$adj_var))) %*%
  bivariate_adj_gbs$gen_adj_var %*%
  solve(chol(bivariate_adj_bs$adj_var))

morph_plot <- ggplot() +
  geom_polygon(
    data = polygon,
    aes(x = -V1, y = V2),
    fill = "#d8d8d8"
  ) +
  geom_vline(aes(xintercept = 0), alpha = 0.5) +
  geom_hline(aes(yintercept = 0), alpha = 0.5) +
  geom_line(data = x_axis, aes(x = -V1, y = V2), size = 1) +
  geom_line(data = y_axis, aes(x = -V1, y = V2), size = 1) +
  geom_point(data = as_tibble(adj_z), aes(x = -V1, y = V2)) +
  geom_point(data = as_tibble(gen_adj_z), aes(x = -V1, y = V2)) +
  geom_path(
    data = bayeslinear::calc_ellipse(
      1, adj_z, matrix(c(1, 0, 0, 1), nrow = 2)
    ),
    aes(x = -x, y = y), alpha = 0.8
  ) +
  geom_path(
    data = bayeslinear::calc_ellipse(
      2, adj_z, matrix(c(1, 0, 0, 1), nrow = 2)
    ),
    aes(x = -x, y = y), alpha = 0.2
  ) +
  geom_path(
    data = bayeslinear::calc_ellipse(
      1, gen_adj_z, gen_adj_var_z
    ),
    aes(x = -x, y = y), alpha = 0.8
  ) +
  geom_path(
    data = bayeslinear::calc_ellipse(
      2, gen_adj_z, gen_adj_var_z
    ),
    aes(x = -x, y = y), alpha = 0.5
  ) +
  scale_x_continuous(
    TeX('$Z_1$'),
    limits = c(x_min, x_max),
    expand = c(0, 0)
  ) +
  scale_y_continuous(
    TeX('$Z_2$'),
    limits = c(y_min, y_max),
    expand = c(0, 0)
  ) +
  geom_text(
    data = adj_z_exp_text,
    aes(x = -x + 0.2, y = y - 0.6, label = text),
    parse = TRUE
  ) +
  geom_text(
    data = gen_adj_z_exp_text,
    aes(x = -x + 0.2, y = y + 0.9, label = text),
    parse = TRUE
  ) +
  coord_fixed() +
  theme(
    panel.grid.minor = element_blank()
  )

adj_exp_plot + cadj_exp_plot + morph_plot
```

# Example - Monotone Regression

We provide a further example with the <tt>cdiac</tt> data that ships with <tt>CVXR</tt>. The data contains Global Monthly and Annual Temperature Anomalies (degrees C, relative to the 1961-1990 mean) for years 1850-2015. This is plotted below.

```{r, echo = FALSE, warning = FALSE, message = FALSE}
data(cdiac)

ggplot(cdiac) +
  geom_point(aes(x = year, y = annual)) +
  xlab("Years") +
  ylab(TeX("Global Temperature Anomaly [$\\degree C$]"))
```

A traditional Bayes linear analysis would specify some prior belief structure and adjust by the data. For example

```{r}
n <- nrow(cdiac)
x <- cdiac$year

dist_mat <- as.matrix(dist(x))

exp_x <- exp_d <- matrix(rep(0, n))

var_x <- 0.3^2 * exp(-(dist_mat / 25)^2) + 1e-6 * diag(rep(1, n))
cov_xd <- 0.3^2 * exp(-(dist_mat / 25)^2) + 1e-6 * diag(rep(1, n))
var_d <- var_x + 0.1^2 * diag(rep(1, n))

monotone <- belief_structure(exp_x, exp_d, cov_xd, var_x, var_d)

d <- cdiac$annual

monotone_adj <- adjust(monotone, d)
```

The result of this adjustment is shown in the left figure, below. We may wish to enforce the belief that the temperatures are monotonically increasing, as a response to monotonic changes in the anthromorphic forcings. We do this as shown.

```{r}
monotone_gbs <- gen_belief_structure(
  exp_x, exp_d, cov_xd, var_x, var_d,
  quote(list(CVXR::diff(gen_adj_exp) >= 0))
)

monotone_gen_adj <- adjust(monotone_gbs, d)
```

This result is shown on the right figure. As before, the plotting code is suppressed, consult the source files if this interests you.

```{r, echo = FALSE}
bl_plot <- tibble(
  year = as.numeric(x),
  adj_exp = as.numeric(monotone_adj$adj_exp),
  adj_sd = sqrt(diag(monotone_adj$adj_var))
) %>%
ggplot() +
  geom_point(data = cdiac, aes(x = year, y = annual)) +
  geom_line(aes(x = year, y = adj_exp)) +
  geom_ribbon(
    aes(x = year, ymin = adj_exp - 2 * adj_sd, ymax = adj_exp + 2 * adj_sd),
    alpha = 0.2
  ) +
  xlab("Years") +
  ylab(TeX("Global Temperature Anomaly [$\\degree C$]")) +
  ggtitle("Standard BL Inference")

gbl_plot <- tibble(
  year = as.numeric(x),
  adj_exp = as.numeric(monotone_gen_adj$gen_adj_exp),
  adj_sd = sqrt(diag(monotone_gen_adj$gen_adj_var))
) %>%
ggplot() +
  geom_point(data = cdiac, aes(x = year, y = annual)) +
  geom_line(aes(x = year, y = adj_exp)) +
  geom_ribbon(
    aes(x = year, ymin = adj_exp - 2 * adj_sd, ymax = adj_exp + 2 * adj_sd),
    alpha = 0.2
  ) +
  xlab("Years") +
  ylab(TeX("Global Temperature Anomaly [$\\degree C$]")) +
  ggtitle("Generalised BL Inference")

```

### The boring stuff

```{r}
sessionInfo()

```











